...
parameter	SOFT_RESET	=	2;			//OV7670 COM7 soft reset (must be first write)
parameter	SET_OV7670	=	3;			//SET_OV LUT Adderss
/////////////////////	Config Data LUT	  //////////////////////////
always@(*)
begin
	case(LUT_INDEX)
    ...
    // ----------------------------------------------------------
    // OV7670 soft reset.  Writing 0x80 to COM7 (0x12) clears all
    // internal registers to their power-on defaults.  Without
    // this, every subsequent register write lands on top of
    // undefined state and the colour / gain pipeline starts up
    // mis-configured.  The remaining ~165 LUT writes that follow
    // take many milliseconds at the 10 kHz SCCB clock, which
    // satisfies the datasheet's >=1 ms post-reset wait.
    // ----------------------------------------------------------
    SOFT_RESET     : 	LUT_DATA	= 	16'h1280;
    ...
    SET_OV7670 + 0 	: 	LUT_DATA	= 	16'h1214;
    	...
    	SET_OV7670 + 7 	: 	LUT_DATA	= 	16'h1717;	// HSTART 0x13 -> 0x17 (drop left blanking)
    	SET_OV7670 + 8 	: 	LUT_DATA	= 	16'h1805;	// HSTOP  0x01 -> 0x05 (slide window, keep width)
